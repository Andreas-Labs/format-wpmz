...
export class XMLParser {
    ...
    private createError(message: string): Error {
        const error = new Error(message) as Error & { position: number, fileName: string };
        ...
        while (this.pos < this.input.length) {
            ...
            if (this.input[this.pos] === '<' && this.input[this.pos + 1] === '/') {
                ...
                if (closingFullTag !== fullTagName) {
                    throw this.createError(`Mismatched closing tag. Expected "${fullTagName}" but got "${closingFullTag}"`);
                }
                // Skip closing '>'
                if (this.input[this.pos] !== '>') {
                    throw this.createError('Expected ">" at position ' + this.pos);
